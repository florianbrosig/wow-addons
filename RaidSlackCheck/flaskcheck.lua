...
if #rscwillcheckthem>0 and rscflaskcheckb[6]==1 then
	for yy=1,#rscwillcheckthem do
		...
		for uu=1,#rscfoodtable do
			--print ("Проверка:"..rscwillcheckthem[yy]..". Спел: "..GetSpellInfo(rscfoodtable[uu]))
			if AuraUtil.FindAuraByName(GetSpellInfo(rscfoodtable[uu]),rscwillcheckthem[yy]) then--rscwillcheckthem[yy]
				local _, _, _, _, _, expirationTime,_, _,_,spell_id,_,_,_,_,_,stats = AuraUtil.FindAuraByName(GetSpellInfo(rscfoodtable[uu]),rscwillcheckthem[yy])
				--print (stats)
    ...
    if byl==0 and rscflaskcheckfoodadd and rscflaskcheckfoodadd==1 then
      for uu=1,#rscfoodtable_additional do
        if AuraUtil.FindAuraByName(GetSpellInfo(rscfoodtable_additional[uu]),rscwillcheckthem[yy]) then
          local _, _, _, _, _, expirationTime,_, _,_,spell_id = AuraUtil.FindAuraByName(GetSpellInfo(rscfoodtable_additional[uu]),rscwillcheckthem[yy])
      ...
      for mm=1,#rscfoodtable do
        if AuraUtil.FindAuraByName(GetSpellInfo(rscfoodtable[mm]),rscwillcheckthem[yy]) then
          local _, _, _, _, _, expirationTime,_, _,_,spell_id = AuraUtil.FindAuraByName(GetSpellInfo(rscfoodtable[mm]),rscwillcheckthem[yy])
    ...
    if byl==0 and rscflaskcheckfoodadd2 and rscflaskcheckfoodadd2==1 then
      for uu=1,#rscfoodtable_additional2 do
        if AuraUtil.FindAuraByName(GetSpellInfo(rscfoodtable_additional2[uu]),rscwillcheckthem[yy]) then
          local _, _, _, _, _, expirationTime,_, _,_,spell_id = AuraUtil.FindAuraByName(GetSpellInfo(rscfoodtable_additional2[uu]),rscwillcheckthem[yy])
	...
	if (byl==0 and byl~=100) then
		if (AuraUtil.FindAuraByName(GetSpellInfo(430),rscwillcheckthem[yy]) or AuraUtil.FindAuraByName(GetSpellInfo(433),rscwillcheckthem[yy])) then
			--персонаж ест
			byl=99
		end
	end
...
if #rscwillcheckthem>0 and rscflaskcheckb[8]==1 then
	for yy=1,#rscwillcheckthem do
		...
		for uu=1,#rscrunestable do
			if AuraUtil.FindAuraByName(GetSpellInfo(rscrunestable[uu]),rscwillcheckthem[yy]) then
				local _, _, _, _, _, expirationTime,_, _,_,spell_id = AuraUtil.FindAuraByName(GetSpellInfo(rscrunestable[uu]),rscwillcheckthem[yy])
...
if #rscwillcheckthem>0 then
	for yy=1,#rscwillcheckthem do
		...
		for uu=1,#rscflasktable do
			if AuraUtil.FindAuraByName(GetSpellInfo(rscflasktable[uu]),rscwillcheckthem[yy]) then
				local _, _, _, _, _, expirationTime,_, _,_,spell_id = AuraUtil.FindAuraByName(GetSpellInfo(rscflasktable[uu]),rscwillcheckthem[yy])
		...
		for uu2=1,#rscflasktableold do
			if AuraUtil.FindAuraByName(GetSpellInfo(rscflasktableold[uu2]),rscwillcheckthem[yy]) then
				local _, _, _, _, _, expirationTime,_, _,_,spell_id = AuraUtil.FindAuraByName(GetSpellInfo(rscflasktableold[uu2]),rscwillcheckthem[yy])
...
if #rscnoflask>0 then
	for yy=1,#rscnoflask do
		...
		for uu=1,#rscelixirtable1 do
			if AuraUtil.FindAuraByName(GetSpellInfo(rscelixirtable1[uu]),rscnoflask[yy]) then
				local _, _, _, _, _, expirationTime,_, _,_,spell_id = AuraUtil.FindAuraByName(GetSpellInfo(rscelixirtable1[uu]),rscnoflask[yy])
...
if #rscnoflask>0 then
	for yy=1,#rscnoflask do
		...
		for uu=1,#rscelixirtable2 do
			if AuraUtil.FindAuraByName(GetSpellInfo(rscelixirtable2[uu]),rscnoflask[yy]) then
				local _, _, _, _, _, expirationTime,_, _,_,spell_id = AuraUtil.FindAuraByName(GetSpellInfo(rscelixirtable2[uu]),rscnoflask[yy])
...
if #rscignorezone15[1]>0 then
	for ug=1,#rscignorezone15[1] do
		if rscignorezone15[1][ug]==curZoneID then
			rscignorezone15[2][ug]=C_Map.GetMapInfo(C_Map.GetBestMapForUnit("player")).name
			mojn=0
		end
	end
end
...
if nn==1 then
	local rrr=0
	if #rscignorezone15[1]>0 then
		for i=1,#rscignorezone15[1] do
			if rscignorezone15[1][i]==curZoneID then
				rscignorezone15[2][i]=C_Map.GetMapInfo(C_Map.GetBestMapForUnit("player")).name
				rrr=1
			end
		end
	end
	if rrr==0 then
		table.insert(rscignorezone15[2],C_Map.GetMapInfo(C_Map.GetBestMapForUnit("player")).name)
		table.insert(rscignorezone15[1],curZoneID)
		out ("|cff99ffffRaidSlackCheck|r - "..rsczonereport2.."|cff00ff00"..C_Map.GetMapInfo(C_Map.GetBestMapForUnit("player")).name.."|r")
	else
	...
	if #rscignorezone15[1]>0 then
		for i=1,#rscignorezone15[1] do
			if rscignorezone15[1][i]==curZoneID then
				...
				table.remove(rscignorezone15[1],i)
				table.remove(rscignorezone15[2],i)
				i=1000
			end
		end
	end
...
if nn==3 then
	local txt=""
	if #rscignorezone15[2]>0 then
		for i=1,#rscignorezone15[2] do
			txt=txt.."|cff00ff00"..rscignorezone15[2][i].."|r"
			if #rscignorezone15[2]>i then
				txt=txt..", "
			end
		end
	end
...
table.wipe(rscignorezone15[1])
table.wipe(rscignorezone15[2])
else
--тыкни еще раз!
